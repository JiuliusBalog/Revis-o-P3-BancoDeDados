...
distanciaPercorrida DECIMAL(7,2) NULL,
...
-- 1)Apresentar marca e modelo de carro e a soma total da distância percorrida pelos
-- carros, em viagens, de uma dada empresa, ordenado pela distância percorrida

SELECT 
	carro.marca,
	carro.modelo, 
	SUM(viagem.distanciaPercorrida) AS TotalDistância
FROM 
	carro
INNER JOIN
	viagem ON carro.id = viagem.idCarro
GROUP BY
	carro.marca,
	carro.modelo
ORDER BY 
...
-- O uso do Group By se dá por causa de colunas individuais vs funções de agregação, o SQL meio que precisa separar as coisas pra
-- ler a distância de cada um
...
SELECT 
	empresa.nome,
	SUM(viagem.distanciaPercorrida) AS DistânciaPorEmpresa
FROM 
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
INNER JOIN
	viagem ON carro.id = viagem.idCarro
GROUP BY
	empresa.nome
HAVING
	SUM(viagem.distanciaPercorrida) > 50000

-- HAVING aparentemente é o WHERE das funções de agregação

-- 3)Apresentar nome das empresas cuja soma total da distância percorrida pelos carros e
-- a media das distâncias percorridas por seus carros em viagens.
-- A média deve ser exibida em uma coluna chamada mediaDist e com 2 casas decimais apenas.
-- Deve-se ordenar a saída pela média decrescente

SELECT
	empresa.nome,
	SUM(viagem.distanciaPercorrida) AS DistânciaPorEmpresa,
	CAST(AVG(viagem.distanciaPercorrida) AS DECIMAL(7,2)) AS mediaDist
FROM
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
...
-- O 10 de Decimal(10,2) é basicamente a quantidade total de valores, exemplo, 1961.851851
-- esse valor (sem contar o ponto) possui 10 casas, queremos 2 casas a direita da virgula, logo Decimal(10,2)

-- 4)Apresentar nome das empresas cujos carro percorreram a maior distância dentre as
-- cadastradas

SELECT
	empresa.nome,
	MAX(viagem.distanciaPercorrida) AS DistânciaMáxima
FROM 
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
...
-- Desde que a empresa tenha 3 ou mais carros. A saída deve ser ordenada pela quantidade
-- de carros, decrescente 

SELECT 
	empresa.nome,
	COUNT(carro.id) AS QtdCarros
FROM 
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
...
LEFT JOIN
	viagem ON carro.id = viagem.idCarro
WHERE
	viagem.idCarro IS NULL

-- 8) Consultar quantas viagens foram feitas por cada carro (marca e modelo) de cada
-- empresa em ordem ascendente de nome de empresa e descendente de quantidade

SELECT
	carro.marca,
	carro.modelo,
	COUNT(viagem.idCarro) AS QtdViagens,
	empresa.nome
FROM
	empresa
INNER JOIN
...
-- e o valor total ganho por viagem, sabendo que para distâncias inferiores a 1000 km, o
-- valor é R$10,00 por km e para viagens superiores a 1000 km, o valor é R$15,00 por km.

SELECT
	empresa.nome,
	carro.marca,
	carro.modelo,
	viagem.distanciaPercorrida,
	CASE
		WHEN (viagem.distanciaPercorrida > 1000) THEN (viagem.distanciaPercorrida * 15)
		WHEN (viagem.distanciaPercorrida <= 1000) THEN (viagem.distanciaPercorrida * 10)
	END AS ValorTotalViagem
FROM
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
...
-- 10) Apresentar o nome da empresa e a média de distância percorrida por seus carros. A
-- saída da média deve ter até 2 casas decimais e deve ser ordenada pela média da distância
-- percorrida

SELECT 
	empresa.nome,
	CAST(AVG(viagem.distanciaPercorrida) AS DECIMAL(7,2)) AS MédiaDistânciaPercorrida
FROM
	empresa
INNER JOIN
	carro ON carro.idEmpresa = empresa.id
INNER JOIN
	viagem ON carro.id = viagem.idCarro
GROUP BY
	empresa.nome
ORDER BY
	MédiaDistânciaPercorrida ASC

-- 11) Apresentar marca e modelo do carro, além do nome da empresa e a data no formato
-- (DD/MM/AAAA) do carro que fez a última viagem dentre os cadastrados

SELECT
	carro.marca,
	carro.modelo,
	empresa.nome,
	CONVERT(VARCHAR, MAX(viagem.data), 103)  AS DataViagem
FROM
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
...
GROUP BY
	...
	empresa.nome
ORDER BY
	MAX(viagem.data) DESC

-- 12) Considerando que hoje é 01/01/2023, apresentar a marca e o modelo do carro, além
-- do nome da empresa e a quantidade de dias da viagem, dos carros que tiveram viagens
-- nos últimos 3 meses. Ordenar (todos ascendentes) por quantidade de dias, marca, modelo
-- e nome da empresa.
...
SELECT
	...
	carro.modelo,
	empresa.nome,
	DATEDIFF(DAY, viagem.data, '2023-01-01') AS QtdDiasViagem
FROM
	empresa
INNER JOIN
	carro ON empresa.id = carro.idEmpresa
INNER JOIN
	viagem ON carro.id = viagem.idCarro
WHERE
	viagem.data >= DATEADD(MONTH, -3, '2023-01-01')
ORDER BY
	QtdDiasViagem ASC,
	carro.marca ASC,
	carro.modelo ASC,
